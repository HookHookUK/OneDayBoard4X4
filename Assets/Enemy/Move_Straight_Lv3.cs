...
public class Move_Straight_Lv3 : MonoBehaviour
{
    [SerializeField] GameObject Obj_X;
    [SerializeField] GameObject Obj_Y;
    int dir = 0;
    ...
    public void PoolInst()
    {
        ...
        if (transform.childCount == 0)
        {
            if(dir < 2)
            {
                temp = Instantiate(Obj_X, setPos, Quaternion.identity);
                temp.transform.SetParent(transform);
            }
            else
            {
                temp = Instantiate(Obj_Y, setPos, Quaternion.identity);
                temp.transform.SetParent(transform);
            }
        }
        else
